...
class WeatherDataService:
    ...
    async def weather_data(self, lat: float, lon: float, requested_dt: datetime) -> WeatherDataPoint | ErrorResponse:
...
class PriceDataService:
    ...
    async def price_data_latest(self) -> List[PriceDataPoint] | ErrorResponse:
        return await self.fetcher.fetch_price_data_latest()
    
    async def price_data_today(self) -> List[PriceDataPoint] | ErrorResponse:
        return await self.fetcher.fetch_price_data_today()
    
    async def price_data_range(self, start_time: str, end_time: str) -> List[PriceDataPoint] | ErrorResponse:
